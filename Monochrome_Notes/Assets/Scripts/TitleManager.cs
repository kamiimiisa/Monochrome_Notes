...
public class TitleManager : MonoBehaviour {
    ...
    [SerializeField] private AudioClip tapTap;
    ...
    void Start() {
        GameMaster.DefoltUiSet(keyboard, joystick);
    ...
    private void PModeChange() {
        if (Input.GetKeyDown(KeyCode.P)) {
            
                GameMaster.HideGameMode = true;    
                tapSource.PlayOneShot(tap);
        }

        if (Input.GetKeyDown(KeyCode.Q)) {
                GameMaster.HideGameMode = false;
                tapSource.PlayOneShot(tapTap);
        }
    }
